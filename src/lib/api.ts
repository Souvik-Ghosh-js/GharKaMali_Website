...
export const rateBooking = (booking_id: number, rating: number, review?: string, geofence_id?: number) =>
  req('/bookings/rate', { method: 'POST', body: JSON.stringify({ booking_id, rating, geofence_id, ...(review ? { review } : {}) }) });
...
export const getBlogCategories = () => req('/blogs/categories', { auth: false });
...
export const AdminAPI = {
  ...
  approveGardener: (id: number, approved: boolean) =>
    req(approved ? '/admin/gardeners/approve' : '/admin/gardeners/reject', { method: 'POST', body: JSON.stringify({ user_id: id }) }),
  ...
  categories: () => req('/admin/shop/categories'),
  createCategory: (form: FormData) => req('/admin/shop/categories', { method: 'POST', body: form }),
  updateCategory: (id: number, form: FormData) => req(`/admin/shop/categories/${id}`, { method: 'PUT', body: form }),
  deleteCategory: (id: number) => req(`/admin/shop/categories/${id}`, { method: 'DELETE' }),

  products: () => req('/admin/shop/products'),
  createProduct: (form: FormData) => req('/admin/shop/products', { method: 'POST', body: form }),
  updateProduct: (id: number, form: FormData) => req(`/admin/shop/products/${id}`, { method: 'PUT', body: form }),
  deleteProduct: (id: number) => req(`/admin/shop/products/${id}`, { method: 'DELETE' }),
...
export const submitReview = (bookingId: number, rating: number, comment?: string, geofence_id?: number) =>
...
// ─── GARDENER WITHDRAWAL ──────────────────────────────────────────────────────
export const requestWithdrawal = (amount: number, geofence_id?: number) =>
  req('/gardener/withdraw', { method: 'POST', body: JSON.stringify({ amount, geofence_id }) });
