  ...
  return (
    <footer style={{
      ...
      <div style={{
        ...
        color: 'rgba(3,65,26,0.03)', letterSpacing: '-0.02em',
        whiteSpace: 'nowrap', pointerEvents: 'none', userSelect: 'none', lineHeight: 1,
      }}>
        GHARKAMALI
      </div>

      <div className="container" style={{ position: 'relative', zIndex: 1, paddingTop: 'clamp(40px, 6vw, 80px)', paddingBottom: 0 }}>
        ...
        <div className="footer-cta-strip" style={{
          ...
          <div>
            <div style={{ fontSize: '0.7rem', letterSpacing: '0.14em', textTransform: 'uppercase', color: 'var(--earth)', fontWeight: 800, marginBottom: 8 }}>Ready to start?</div>
            <h3 style={{ fontFamily: 'var(--font-display)', fontSize: 'clamp(1.1rem,3vw,2rem)', fontWeight: 800, color: 'var(--forest)', margin: 0 }}>
              Your garden deserves <span style={{ color: 'var(--earth)' }}>the best</span>
            </h3>
          </div>
          <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap', flex: '0 0 auto' }}>
            <Link href="/book" className="btn btn-primary" style={{ display: 'flex', alignItems: 'center', gap: 8, padding: 'clamp(12px,2vw,16px) clamp(20px,3vw,36px)', whiteSpace: 'nowrap' }}>
              Book Now <IcArrow />
            </Link>
            <a href={WA_URL} target="_blank" rel="noopener noreferrer"
              style={{ display: 'flex', alignItems: 'center', gap: 8, padding: 'clamp(12px,2vw,16px) clamp(20px,3vw,36px)', background: '#fff', border: '2px solid var(--gold)', borderRadius: 99, fontSize: 'clamp(0.8rem,1.2vw,0.9rem)', fontWeight: 800, color: 'var(--forest)', textDecoration: 'none', transition: 'all 0.3s', whiteSpace: 'nowrap' }}
            ...
            <div className="footer-social-row" style={{ display: 'flex', flexDirection: 'row', gap: 10, marginTop: 4, flexWrap: 'nowrap', justifyContent: 'center' }}>
              {[
                ...
                <a key={s.label} href={s.href} target="_blank" rel="noopener noreferrer" aria-label={s.label}
                  style={{ width: 42, height: 42, borderRadius: 11, background: 'var(--bg)', border: '1px solid var(--border)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--forest)', transition: 'all 0.3s', flexShrink: 0 }}
                  onMouseEnter={e => { (e.currentTarget as HTMLElement).style.background = 'var(--bg-elevated)'; (e.currentTarget as HTMLElement).style.borderColor = 'var(--gold)'; (e.currentTarget as HTMLElement).style.transform = 'translateY(-3px)'; }}
                  onMouseLeave={e => { (e.currentTarget as HTMLElement).style.background = 'var(--bg)'; (e.currentTarget as HTMLElement).style.borderColor = 'var(--border)'; (e.currentTarget as HTMLElement).style.transform = ''; }}>
                  <s.Icon />
                </a>
              ))}
            </div>
          ...
          {NAV_COLS.map(col => (
            <div key={col.title}>
              <div style={{
                ...
                color: 'var(--forest)', marginBottom: 20,
              }}>{col.title}</div>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                {col.links
                  .filter(l => SHOP_ENABLED || !l.href.startsWith('/shop'))
                  .map(l => <FooterLink key={l.href} href={l.href} label={l.label} />)}
              </div>
            </div>
          ))}
        </div>
        ...
        {/* Divider */}
        <div style={{ height: 1, background: 'linear-gradient(90deg, transparent 0%, var(--border-mid) 50%, transparent 100%)', marginBottom: 28 }} />
