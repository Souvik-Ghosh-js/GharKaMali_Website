    ...
    <>
      ...
      <div style={{ paddingTop:'var(--nav-h)', background:'var(--bg)', minHeight:'100svh', display:'flex', alignItems:'center', justifyContent:'center' }}>
        <div style={{ textAlign:'center' }}>
          ...
          <h2 style={{ fontFamily:'var(--font-display)', fontWeight:800, fontSize:'1.8rem', marginBottom:16, color:'var(--forest)' }}>Article not found</h2>
      ...
      <div style={{ background:'var(--bg)', paddingTop:'var(--nav-h)', minHeight:'100svh' }}>
        ...
        <div style={{ height:'clamp(300px,45vw,500px)', overflow:'hidden', position:'relative' }}>
          ...
          <div className="container" style={{ position:'absolute', bottom:36, left:'50%', transform:'translateX(-50%)', width:'100%' }}>
            {b.category && <span style={{ display:'inline-block', padding:'5px 14px', borderRadius:99, background:'rgba(255,255,255,0.15)', backdropFilter:'blur(8px)', color:'#fff', fontSize:'0.72rem', fontWeight:700, marginBottom:12, textTransform:'uppercase', letterSpacing:'0.07em' }}>{b.category.name || b.category}</span>}
        ...
        <div className="container" style={{ paddingBottom:80 }}>
          <div style={{ display:'grid', gridTemplateColumns:'1fr 280px', gap:40, marginTop:48, alignItems:'start' }}>
            ...
            <div>
              ...
              {b.content && (
                <div style={{ fontSize:'1.05rem', lineHeight:1.85, color:'var(--forest)' }}
              ...
              {b.tags?.length > 0 && (
                <div style={{ marginTop:40, display:'flex', gap:8, flexWrap:'wrap' }}>
                  {b.tags.map((tag: any) => (
                    <span key={tag.slug || tag} style={{ padding:'5px 14px', borderRadius:99, background:'rgba(11,61,46,0.08)', color:'var(--forest)', fontSize:'0.78rem', fontWeight:600 }}>#{tag.name || tag}</span>
                  ))}
                </div>
              )}
            ...
            <div style={{ position:'sticky', top:'calc(var(--nav-h) + 24px)' }}>
              {more.length > 0 && (
                <div className="card" style={{ borderRadius:22, padding:'24px' }}>
                  <h3 style={{ fontWeight:800, fontSize:'1rem', marginBottom:20, color:'var(--forest)', fontFamily:'var(--font-display)' }}>More Articles</h3>
                  <div style={{ display:'flex', flexDirection:'column', gap:16 }}>
                    {more.map((m: any) => (
                      <Link key={m.id} href={`/blogs/${m.slug}`}
                        ...
                        <div style={{ width:56, height:56, borderRadius:14, overflow:'hidden', flexShrink:0, background:'var(--bg-elevated)', border:'1px solid var(--border)' }}>
                          {m.cover_image ? <img src={m.cover_image} alt="" style={{ width:'100%', height:'100%', objectFit:'cover' }} /> : <div style={{ height:'100%', display:'flex', alignItems:'center', justifyContent:'center', fontSize:'1.5rem', color:'var(--sage)' }}></div>}
                        </div>
                        <div style={{ flex:1, minWidth:0 }}>
                          <div style={{ fontWeight:700, fontSize:'0.88rem', lineHeight:1.4, color:'var(--forest)', overflow:'hidden', display:'-webkit-box', WebkitLineClamp:2, WebkitBoxOrient:'vertical' }}>{m.title}</div>
                          <div style={{ fontSize:'0.75rem', color:'var(--earth)', marginTop:6, fontFamily:'var(--font-mono)', fontWeight:600 }}>{m.created_at && new Date(m.created_at).toLocaleDateString('en-IN', { day:'numeric', month:'short' })}</div>
                        </div>
                      </Link>
                    ))}
                  </div>
