...
export default function ShopPage() {
  ...
  useEffect(() => {
    ...
    const t = setTimeout(() => {
      getShopProducts(params).then((d: any) => setProducts(Array.isArray(d) ? d : [])).catch(() => setProducts([])).finally(() => setLoading(false));
    }, 250);
    ...
    <>
      ...
      <div style={{ background: 'var(--bg)', paddingTop: 'var(--nav-h)', minHeight: '100svh' }}>
        ...
        <div className="container" style={{ paddingTop: 32, paddingBottom: 80 }}>
          {/* Category tabs */}
          <div className="cat-tabs-row" style={{ display: 'flex', gap: 8, overflowX: 'auto', paddingBottom: 8, marginBottom: 32, scrollbarWidth: 'none' }}>
            {categories.map(c => {
