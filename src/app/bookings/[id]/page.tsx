    ...
    <>
      ...
      <div style={{ background: 'var(--bg)', minHeight: '100svh', position: 'relative' }}>
          ...
          <div className="container" style={{ position: 'relative', zIndex: 2 }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: 16 }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: 12 }}>
                ...
                {booking.booking_type !== 'subscription' && Number(booking.total_amount) > 0 && (
                  <button onClick={() => downloadBookingBill(booking)}
                    style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '10px 18px', background: '#fff', color: 'var(--forest)', border: 'none', borderRadius: 12, fontWeight: 800, fontSize: '0.82rem', cursor: 'pointer', fontFamily: 'var(--font-body)', boxShadow: '0 4px 14px rgba(0,0,0,0.18)' }}>
        ...
        <div className="container" style={{ marginTop: -44, paddingBottom: 80 }}>
          <div className="booking-detail-grid" style={{ display: 'grid', gridTemplateColumns: '1fr 340px', gap: 24, alignItems: 'start' }}>
            {/* Left */}
            <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
              {/* Live OTP card — show when booking is active */}
              {['assigned', 'en_route', 'arrived'].includes(status) && booking.otp && (
                <div className="booking-otp-panel" style={{ background: 'linear-gradient(135deg, var(--forest), var(--forest-mid))', borderRadius: 24, padding: 28, animation: 'slide-up 0.4s var(--ease)' }}>
                  <div style={{ fontSize: '0.7rem', fontWeight: 700, color: 'rgba(255,255,255,0.5)', letterSpacing: '0.1em', textTransform: 'uppercase', marginBottom: 12, display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
                    <IcLock /> Share this OTP with your gardener to start the visit
                  </div>
                  <div className="booking-otp-digits" style={{ display: 'flex', gap: 8, marginBottom: 16, flexWrap: 'wrap' }}>
                    {booking.otp.toString().split('').map((d: string, i: number) => (
                      <div key={i} style={{ width: 52, height: 62, borderRadius: 14, background: 'rgba(255,255,255,0.12)', border: '1px solid rgba(255,255,255,0.2)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: 'monospace', fontSize: '2rem', fontWeight: 900, color: 'var(--gold-light)', flexShrink: 0 }}>{d}</div>
                    ))}
                  </div>
            ...
            {/* Right sidebar */}
            <div className="booking-detail-sidebar" style={{ position: 'sticky', top: 'calc(var(--nav-h) + 20px)', display: 'flex', flexDirection: 'column', gap: 14 }}>
              {/* Booking details */}
              <div style={{ background: 'var(--bg-card)', borderRadius: 24, padding: '22px 24px', border: '1px solid var(--border)' }}>
