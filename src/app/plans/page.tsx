...
const DEFAULT_FAQS = [
  ...
  { q: 'Are your plant experts background-verified?', a: 'Absolutely. Every plant expert goes through a thorough background check, skill assessment, and training program before being assigned to customers.' },
  { q: "What if I'm not satisfied with the visit?", a: "We offer a 100% satisfaction guarantee. If you're not happy, we'll send a replacement plant expert free of charge within 24 hours." },
  { q: 'How does the live tracking work?', a: "Once your plant expert starts their journey, you'll get a live map link showing their real-time location. You'll also receive WhatsApp updates at every milestone." },
  { q: 'Can I request the same plant expert every visit?', a: 'Yes, on our Premium plan you get a dedicated plant expert who learns your garden over time. Standard and Basic plans try to maintain consistency but cannot guarantee the same plant expert.' },
];
  ...
  if (isMobile) {
    return (
      <div style={{ margin: '0 -20px', padding: '20px 0 32px' }}>
        <div style={{ display: 'flex', overflowX: 'auto', gap: 16, padding: '8px 20px 16px', scrollSnapType: 'x mandatory', WebkitOverflowScrolling: 'touch', scrollbarWidth: 'none' }}>
              ...
              <div key={plan.id || i} style={{ flexShrink: 0, width: 'calc(100vw - 60px)', maxWidth: 360, scrollSnapAlign: 'center', background: isDark ? 'var(--forest)' : '#fff', borderRadius: 28, border: `1.5px solid ${isDark ? 'transparent' : 'var(--border-gold)'}`, padding: '32px 24px', boxShadow: isDark ? 'var(--sh-xl)' : 'var(--sh-md)', position: 'relative' }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 10, marginBottom: 28 }}>
                  {(Array.isArray(plan.features) ? plan.features : []).slice(0, 4).map((f: string, idx: number) => (
                    <div key={idx} style={{ display: 'flex', alignItems: 'center', gap: 10, fontSize: '0.85rem', color: isDark ? 'rgba(255,255,255,0.8)' : 'var(--forest)', fontWeight: 600 }}>
                      <span style={{ color: isDark ? 'var(--gold)' : 'var(--forest)', opacity: 0.8 }}><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3.5"><polyline points="20 6 9 17 4 12"/></svg></span>
                      {f}
                    </div>
                  ))}
                </div>
  ...
  return (
    <SmoothScrollProvider>
      ...
      <style jsx global>{`
        ...
        /* Plans page: hide particle dots on mobile */
        @media (max-width: 767px) {
          .plans-hero-canvas { display: none !important; }
        }
        /* Compare table: always scrollable on mobile */
        .plans-compare-table { table-layout: auto; }
        @media (max-width: 640px) {
          .guarantee-premium { flex-direction: column !important; align-items: flex-start !important; }
          .guarantee-icon { width: 56px !important; height: 56px !important; }
        }
      `}</style>
    </SmoothScrollProvider>
  );
