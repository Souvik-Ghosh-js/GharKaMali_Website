...
function StatusPill({ s }: { s: string }) {
  ...
  return <span style={{ padding:'4px 12px', borderRadius:99, fontSize:'0.7rem', fontWeight:800, textTransform:'uppercase', letterSpacing:'0.06em', background:bg, color, border: `1px solid ${color}40` }}>{s.replace(/_/g,' ')}</span>;
}
  ...
  return (
    <>
      ...
      <div style={{ minHeight:'calc(100svh - var(--nav-h))', background:'var(--bg)', paddingTop:'var(--nav-h)', position: 'relative' }}>
              ...
              <div>
                ...
                <div style={{ display:'flex', gap:12, flexWrap:'wrap' }}>
                  {[
                    ...
                    <div key={stat.label} style={{ background:'#fff', border:(stat.hi ? '2px solid var(--gold)' : '1px solid var(--border-mid)'), borderRadius:16, padding:'16px 24px', boxShadow: 'var(--sh-xs)' }}>
        ...
        <div className="container" style={{ position: 'relative', zIndex: 11, marginTop:-44, paddingBottom:72 }}>
                ...
                <div style={{ display:'flex', flexDirection:'column', gap:14 }}>
                  {bLoad ? Array(3).fill(null).map((_,i) => (
                    ...
                    <Link key={b.id} href={`/bookings/${b.id}`}
                      className="card"
                      style={{ borderRadius:24, padding:'20px 24px', textDecoration:'none', display:'flex', alignItems:'center', gap:16 }}>
                    ...
                    <Link key={o.id} href="/shop/orders"
                      ...
                      <div style={{ flex:1, minWidth:0 }}>
                        <div style={{ display:'flex', alignItems:'center', gap:10, marginBottom:8 }}>
                          ...
                          <span style={{ fontSize:'0.7rem', fontWeight:900, textTransform:'uppercase', color:'var(--earth)', background:'var(--bg-elevated)', padding:'4px 10px', borderRadius:8, border: '1px solid var(--border-gold)' }}>{o.status}</span>
                        </div>
              ...
              {activeSub ? (
                <div style={{ background:'var(--forest)', borderRadius:32, padding:'clamp(30px,4vw,40px)', position:'relative', overflow:'hidden', border: '2px solid var(--border)', boxShadow: 'var(--sh-lg)' }}>
                  ...
                  <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center' }}>
                    <span style={{ fontFamily:'var(--font-display)', fontSize:'2.2rem', fontWeight:900, color:'var(--gold)' }}>₹{activeSub.plan?.price?.toLocaleString('en-IN')}<span style={{ fontSize:'0.9rem', color:'rgba(255,255,255,0.7)', fontWeight:600 }}>/mo</span></span>
                    <Link href="/subscriptions" style={{ background:'#fff', color:'var(--forest)', padding:'12px 24px', borderRadius:99, fontSize:'0.9rem', fontWeight:800, textDecoration:'none', boxShadow: 'var(--sh-sm)' }}>Manage</Link>
                  </div>
                </div>
              ) : (
